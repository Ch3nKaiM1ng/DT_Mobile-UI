    ...
    >
      <div class="comment_top2" v-for="(j,k) in comment3List" :key="k">
        <div class="comment_center2">
          ...
          <div class="comment_loveimg2" @click="art_CommDianZan(j,$event)">
...
export default {
  ...
  data() {
    return {
      ...
      artAllComData: { artId: 0, level: 1, limit: 1000, offset: 1, topId: 0 }
    };
  },
  created() {
    this.reply2.artId = this.$route.query.artId;
    ...
    this.artAllComData.artId = this.$route.query.artId;
    this.artAllComData.topId = this.$route.query.topId;
    this.$request.artAllCom(this.artAllComData).then(res => {
  ...
  methods: {
    ...
    pinglun() {
      if (this.value !== "") {
        ...
        this.value = "";
        this.$request.artDetailReply(this.reply2).then(res => {
          this.$request.artAllCom(this.artAllComData).then(res => {
            this.comment3List = res.data.data.dataList;
            this.$nextTick(() => {
              setTimeout(() => {
                let h = $("#commentArt")[0].scrollHeight;
                $("#commentArt").scrollTop(h);
              }, 300);
            });
          });
        });
...
<style lang="scss">
@import "../assets/css/commentArt.scss";
</style>